...
class FreqExtr_Module(ExternalModule):
	...
	self.applyEffortService = None
        ...
        self.weights_sub = rospy.Subscriber("/weights", Vector3, self.weightsCallback, queue_size=1)
    ...
    def run_step(self):
        ...
        if self.joint_states is not None:
            ...
            if self.applyEffortService.value is None:
                self.applyEffortService.value = rospy.ServiceProxy("/gazebo/apply_joint_effort", ApplyJointEffort)
            if self.clearJointService.value is None:
                self.clearJointService.value = rospy.ServiceProxy("/gazebo/clear_joint_forces", JointRequest)
